package net.kirill.MineralAtlas.advancement;
	...
	public void generate(final AdvancementOutput output, final Provider registries) {
		...
		final var root = AdvancementBuilder.root()
				.display(SimpleDisplayInfo.builder()
						.icon(MineralAtlasModItems.EXAMPLE_ITEM)
				...
				.addCriterion("unlocked_example_item_recipe",
						RecipeUnlockedTrigger.unlocked(MineralAtlas.location("example_recipe_folder/custom_recipe_name")))
				.save(output, MineralAtlas.location("example/root"));
		...
		AdvancementBuilder.child(root)
				...
				.addCriterion("obtained_example_item", InventoryChangeTrigger.TriggerInstance.hasItems(MineralAtlasModItems.EXAMPLE_ITEM))
				.save(output, MineralAtlas.location("example/child"));
	}
}