...
import net.kirill.MineralAtlas.common.item.*;
...
public class BuiltInLanguage extends EnhancedLanguageProvider {
	...
	public BuiltInLanguage(final PackOutput output, final ExtraLanguageProvider... extraLanguageProviders) {
		super(output, MineralAtlas.MOD_ID, "en_us", extraLanguageProviders);
	}
...
//		add(LanguageTranslation.item(ExampleModItems.EXAMPLE_ITEM, "Example Item"));
		add(LanguageTranslation.simpleItem(MineralAtlasModItems.EXAMPLE_ITEM));

		add(LanguageTranslation.of(MineralAtlasItem.EXAMPLE_TOOLTIP, "Example Tooltip"));
	}

	@Override
	protected Stream<KnownRegistryContents<?>> knownRegistryContents() {
		return Stream.of(KnownRegistryContents.item(MineralAtlasModItems.ITEMS));
	}
}