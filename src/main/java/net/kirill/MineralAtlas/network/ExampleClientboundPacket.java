package net.kirill.MineralAtlas.network;
...
public record ExampleClientboundPacket(int entityId) implements CustomPacketPayload {
	...
	public static final Type<ExampleClientboundPacket> TYPE = new Type<>(MineralAtlas.location("example_client_packet"));
	...
	void handle() {
		...
		if (entity == ClientHelpers.getPlayer()) {
			MineralAtlas.LOG.debug("We received a packet for our own player entity!");
		}
	}
